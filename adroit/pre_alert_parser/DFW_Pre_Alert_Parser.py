...
class Pre_Alert_Parser():
    def __init__(self, mongo_host="", mongo_port=27017, root_path="/apps/webapps/utilites/DFW_prealerts_parsing"):
        print "Parsing start......"
        self.mongo_host = mongo_host
        self.mongo_port = mongo_port
        self.root_path = root_path        
        self.dbObj = self.getMongoConn()
#        self.dbObj1 = self.getMongoConn1()
        self.bb_attachments = gridfs.GridFS(self.dbObj,"bb_attachments")
...
#			pass
    ...
    def insertIntoDB(self, dict, mail):
        conn = MySQLdb.connect(host="",user = "",passwd = "",db="ADROIT")
        
        #conn = MySQLdb.connect(host="localhost",user = "root",passwd = "",db="myadroittestdb")
#        conn = MySQLdb.connect(host="122.248.234.221",user = "adroit_rw",passwd = "adroit#rw12",db="ADROIT_TEST")     
        ...
        if not row:
            ...
            cursor = conn.cursor()
            cursor.execute(""" INSERT INTO process_it_mawbdetail (papFlag, pamFlag, mawbNum, station, preAlertReceivedDate, preAlertReceivedTime, lastUpdatedDate, lastUpdatedTime) VALUES (%s, %s, %s, %s, %s, %s, %s, %s) """,(True, False, dict['Mawb'], 'DFW/U', datetime.strptime(dict['date'], "%Y%m%d%H%M%S").strftime("%Y-%m-%d"), datetime.strptime(dict['date'], "%Y%m%d%H%M%S").strftime("%H:%M:%S"), datetime.strptime(dict['date'], "%Y%m%d%H%M%S").strftime("%Y-%m-%d"), datetime.strptime(dict['date'], "%Y%m%d%H%M%S").strftime("%H:%M:%S")))
            cursor.close()
            conn.commit()
            conn.close()
        else:            
            dict['duplicate'] = '1'
            dict['categories'] = self.dbObj.ds.find({"Mawb":dict['Mawb']})[0]['categories']
            dict['categories'].append('blue')            
            
        dict = self.mailProcess(dict,mail)
        dict['attachments_list'] = list(set(dict['attachments_list']))
        self.dbObj.ds.insert(dict,force_insert=True)
...
#        pdb.set_trace()
        ...
        print M.getwelcome()
        print M.user("dfw.bb.air.hcl.ceva@searce.com")
        print M.pass_("Success@888")
        #print '/////////////////////////////',M.list()[1]
        print '/////////////////////////////',M.uidl()[1]
        ...
        for i in range(0,numMessages):            
            ...
            if self.dbObj.ds.find({"uidl":M.uidl()[1][i].split(' ')[1]}).count() > 0:
                print "in uidl"
                continue
            dict = {}
            dict1 = {}
...
#                pdb.set_trace()
                ...
                if subject.find('Air ETA') == 0:
                    ...
                    if mail.is_multipart():
                        ...
                        flg = 1
#                        conn = MySQLdb.connect(host="localhost",user = "adroit_rw",passwd = "adroit#rw12",db="ADROIT")
                        #conn = MySQLdb.connect(host="122.248.234.221",user = "adroit_rw",passwd = "adroit#rw12",db="ADROIT_TEST") 
                        conn = MySQLdb.connect(host="",user = "",passwd = "",db="ADROIT")
                        cursor = conn.cursor()
                        cursor.execute ("SELECT 1 FROM process_it_mawbdetail WHERE mawbNum='%s'" % (dict['Mawb']))
                        ...
                        #print '==', dict
                        if not row:
                            if dict['station'] == 'DFG' or dict['station'] == 'MSY' :
#                                if  (data[data.rfind('MAWB')+5:data.rfind('MAWB')+9]).strip(" ") == "150" :
                                scope_flag = False
                            else:
                                scope_flag = True
...
#                            print "-------------inserted-------------MAWB",dict['Mawb'],"=====STATION==", dict['station']
                            cursor.close()
                            conn.commit()
                            conn.close()
                        ...
                        if dict['station'] == 'DFW':
                            dict['categories'].append('blue')
                        elif dict['station'] == 'DFG' or dict['station'] == 'MSY':
                            dict['categories'].append('black')
